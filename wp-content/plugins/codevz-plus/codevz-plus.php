...
class Codevz_Plus {
	...
	// Plugin version.
	public static $ver = '3.8.5';
	
	// Server API address.
	public static $api = 'https://xtratheme.com/plugins/';
	...
	public function __construct() {
		...
		require_once( self::$dir . 'classes/class-minifier.php' );
		...
		if( ! self::$vc_editable && self::option( 'lazyload' ) ) {
			...
			add_filter( 'wp_nav_menu_items', $lazyload_method, 999 );
	...
	public function share() {
		...
		foreach( $share as $name ) {
			...
			if ( isset( $url[ $name ] ) ) {
				...
				$title_prefix = ( $name === 'envelope' ) ? esc_html__( 'Share by', 'codevz' ) : esc_html__( 'Share on', 'codevz' );

				echo '<a href="' . $url[ $name ]['url'] . '" class="cz-' . $name . '" ' . ( $tooltip ? 'data-' : '' ) . 'title="' . esc_attr( $title_prefix . ' ' . $url[ $name ]['title'] ) . '"><i class="fa fa-' . $name . '"></i><span>' . esc_html( $url[ $name ]['title'] ) . '</span></a>';
	...
	public static function tiny_mce_before_init( $i ) {
		...
		$style_formats = array(
			...
			array(
				'title' => esc_html__( 'Letter spacing 0px', 'codevz' ),
				'inline' => 'span',
				'wrapper' => false,
	...
	public static function ajax_search() {
		...
		if ( $q->post_count >= $l ) {
			unset( $_GET['action'] );
			unset( $_GET['nonce'] );
			echo '<a class="va_results" href="' . esc_url( home_url( '/' ) ) . '?' . http_build_query( $_GET ) . '"> ... </div>';
		}
	...
	public static function get_image( $i = '', $s = 0, $url = 0, $c = '' ) {
		...
		if( empty( $i ) ) {
			...
			$i = '<img src="' . self::$url . 'assets/img/p.svg' . '" class="' . $c . '" width="1000" height="1000" alt="image" />';
