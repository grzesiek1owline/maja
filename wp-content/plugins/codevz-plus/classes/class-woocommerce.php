...
class Xtra_Woocommerce {
			        		...
			        		?>
					            <div class="item_small">
					                ...
					                <div class="cart_list_product_title">
					                    ...
					                    <a href="<?php echo esc_url( wc_get_cart_remove_url( $cart_item_key ) ); ?>" class="remove" data-product_id="<?php echo esc_attr( $id ); ?>"><i class="fa czico-198-cancel"></i></a>
					                </div>
					            </div>
			        		<?php } ?>
			        	</div>
	...
	public static function single_wishlist() {
		...
		if ( Codevz_Plus::option( 'woo_wishlist' ) ) {
			...
			echo '<div class="xtra-product-icons" data-id="' . get_the_ID() . '">';
			echo '<i class="fa fa-heart-o xtra-add-to-wishlist" data-tooltip="' . esc_html__( 'Add to Wishlist', 'codevz' ) . '"></i>';
			echo '</div>';
		}
	...
	public static function woocommerce_before_shop_loop_item_title_low() {
		...
		if ( $wishlist || $quick_view ) {
			...
			echo $wishlist ? '<i class="fa fa-heart-o xtra-add-to-wishlist" data-tooltip="' . esc_html__( 'Add to Wishlist', 'codevz' ) . '"></i>' : '';
			echo $quick_view ? '<i class="fa czico-147-magnifying-glass xtra-product-quick-view" data-tooltip="' . esc_html__( 'Quick View', 'codevz' ) . '" data-nonce="' . wp_create_nonce( 'xtra_quick_view' ) . '"></i>' : '';
			echo '</div>';
